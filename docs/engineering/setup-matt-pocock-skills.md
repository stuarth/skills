...
- **Issue tracker** — where work is tracked, so `triage`, `to-spec`,
  `to-tickets`, `wayfinder`, and `through-line` know whether to call `gh`, `glab`,
  write markdown under `.scratch/`, or follow a workflow you describe. GitHub,
  GitLab, local markdown, or other. (It proposes the one that matches your
  `git remote`.)
