...
It **does no work itself**. It doesn't grill, write a spec, or fix anything — it only orients. It exists for the **user-invoked** skills above all: nothing fires those for you, so *you* have to remember they exist, and `ask-matt` is the memory you offload that to. That includes distinguishing the two huge-effort flows: [wayfinder](https://aihero.dev/skills-wayfinder) clears a route one decision at a time, while [through-line](https://aihero.dev/skills-through-line) distills reusable principles and propagates each verdict through the remaining map so only residual judgment returns to you. When that map includes execution, its Work session coordinates bounded executors while retaining review and propagation. It also points at the model-invoked skills you'd reach for by name — `/tdd`, `/diagnosing-bugs`, `/prototype`, `/code-review`, and the two vocabulary references, `/domain-modeling` and `/codebase-design`. It answers "which one, and when", then hands you off to the skill that actually does the job.
...
The idea `ask-matt` gives you to think with is the **flow** — a path *through* the skills rather than a single one. Most work runs along one **main flow** (idea → ship: grill → spec → tickets → implement → review), with on-ramps for incoming work, hard bugs, codebase health, and huge efforts whose route or principles still need to be found. Everything else is a **standalone** you reach for on its own. Ask a question and you get placed on the right flow, at the right step — not just handed a tool.

## Where it fits

`ask-matt` is the **router** — the standalone map that sits over the whole set. It is the node every other docs page links back to as [ask-matt](https://aihero.dev/skills-ask-matt), so it never sits *in* a chain; it points *into* every chain. From here you'll most often land on [grill-with-docs](https://aihero.dev/skills-grill-with-docs), the head of the main flow; [triage](https://aihero.dev/skills-triage), the on-ramp for work you didn't create; or [wayfinder](https://aihero.dev/skills-wayfinder) and [through-line](https://aihero.dev/skills-through-line) for efforts too large to hold in one session. When even the router's own picture is stale, its [Source](https://github.com/mattpocock/skills/tree/main/skills/engineering/ask-matt) is the map of record.
